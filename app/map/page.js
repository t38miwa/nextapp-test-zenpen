...
const MapComponent = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      const items = await getSingleItem();
      setSingleItem(items);
    }
    fetchData();
  }, []);
    ...
    service.getDetails({
      ...
      if (status === window.google.maps.places.PlacesServiceStatus.OK) {
        setSelectedPlace({
          ...
          name: result.name || '情報がありません',
          info: result.formatted_address || '住所情報がありません',
          phone: result.formatted_phone_number || '電話番号がありません',
          website: result.website || 'ウェブサイトがありません',
          photoUrl: result.photos ? result.photos[0].getUrl() : '画像がありません'
        });
      }
    });
  }, []);
          ...
          {selectedPlace && (
            <InfoWindow position={selectedPlace.position} onCloseClick={() => setSelectedPlace(null)}>
              <div className="map-container">
                ...
                {selectedPlace.phone && <p>Phone: {selectedPlace.phone}</p>}
                {selectedPlace.website && <p><a href={selectedPlace.website} target="_blank" rel="noopener noreferrer">Website</a></p>}
                {selectedPlace.photoUrl && <img src={selectedPlace.photoUrl} alt="Place" />}
              </div>
            </InfoWindow>
          )}
        </GoogleMap>
